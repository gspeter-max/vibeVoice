...
class AudioSpectrogramConverter:
    ...
    def __init__(self, model_configuration, filterbank_file_path):
        ...
        self.mathematical_window = get_window(
            self.settings["window"], self.settings["win_length"], fftbins=True
        )

        # Load the frequency map (filterbank) from a binary file
        self.frequency_map = np.fromfile(filterbank_file_path, dtype=np.float32).reshape(
            1, 128, 257
        )[0]
    ...
    def convert_sound_to_spectrogram(self, raw_sound_data):
        ...
        processed_sound = np.append(
            raw_sound_data[0], raw_sound_data[1:] - self.settings["preemph"] * raw_sound_data[:-1]
        )

        # Step 2: Cut the sound into small overlapping pieces
        number_of_frames = (
            1 + (len(processed_sound) - self.settings["win_length"]) // self.settings["hop_length"]
        )
        sound_frames = np.lib.stride_tricks.as_strided(
            processed_sound,
            shape=(number_of_frames, self.settings["win_length"]),
            strides=(
                processed_sound.strides[0] * self.settings["hop_length"],
                processed_sound.strides[0],
            ),
        )

        # Step 3: Use a mathematical formula (FFT) to find the volume of different pitches
        pitch_volumes = (
            np.abs(np.fft.rfft(sound_frames * self.mathematical_window, n=self.settings["n_fft"]))
            ** 2
        )
...
class NemotronStreamingEngine:
    ...
    def __init__(self):
        ...
        self.models_directory = os.path.expanduser(
            "~/.cache/parakeet-flow/models/nemotron-0.6b-onnx"
        )
        ...
        with open(tokens_file_path, "r", encoding="utf-8") as tokens_file:
            self.vocabulary_tokens = [
                line.strip().split()[0] for line in tokens_file if line.strip()
            ]
    ...
    def clear_internal_memory(self):
        ...
        self.encoder_memory = {
            "cache_last_channel": np.zeros(
                encoder_settings["cache_last_channel_shape"], dtype=np.float32
            ),
            "cache_last_time": np.zeros(
                encoder_settings["cache_last_time_shape"], dtype=np.float32
            ),
            "cache_last_channel_len": np.array([0], dtype=np.int64),
        }

        self.decoder_memory = {
            "input_states_1": np.zeros(
                (2, 1, decoder_settings["prediction_hidden"]), dtype=np.float32
            ),
            "input_states_2": np.zeros(
                (2, 1, decoder_settings["prediction_hidden"]), dtype=np.float32
            ),
        }
    ...
    def add_audio_chunk_and_get_text(self, new_sound_data):
        ...
        combined_spectrogram = np.concatenate(
            [self.previous_sound_frames, current_spectrogram], axis=2
        )
        self.previous_sound_frames = current_spectrogram[
            :, :, -9:
        ]  # Save the end of this picture for next time
        ...
        for time_step in range(encoded_frames.shape[2]):
            ...
            for _ in range(max_words_per_frame):
                ...
                self.decoder_memory.update(
                    {"input_states_1": decoder_outputs[2], "input_states_2": decoder_outputs[3]}
                )
