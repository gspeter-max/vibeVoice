...
from src.text_refiner.llm_router import llm_refine
...
def test_router_rotates_on_failure(mock_env_get, mock_call_api, mock_check_key):
    ...
    result1 = router.llm_refine("hello")
    ...
    result2 = router.llm_refine("world")
...
def test_router_full_rotation(mock_env_get, mock_call_api, mock_check_key):
    ...
    router.llm_refine("1")  # Groq fails → index 1 (Cerebras)
    router.llm_refine("2")  # Cerebras fails → index 2 (OpenGateway)
    router.llm_refine("3")  # OpenGateway fails → index 0 (Groq)
