...
def test_parse_incoming_message_handles_switch_model():
    ...
    result = parse_msg(raw_message)
    assert result == {"command_type": "switch_model", "model_name": "nemotron-v1"}


def test_parse_incoming_message_handles_edge_cases():
    # 1. Malformed Audio Chunk (missing parts)
    res = parse_msg(b"CMD_AUDIO_CHUNK:session123:0")
    ...
    res = parse_msg(b"CMD_SESSION_EVENT:session123\n\n{}")
    assert res["command_type"] == "error"
    assert res["reason"] == "bad_session_event_format"

    # 4. Malformed Session Commit (bad parts count)
    res = parse_msg(b"CMD_SESSION_COMMIT:session123:0:extra")
    assert res["command_type"] == "error"
    assert res["reason"] == "bad_session_commit_format"

    # 5. Empty message
    assert parse_msg(b"")["command_type"] == "raw_audio"
    assert parse_msg(b"")["payload_bytes"] == b""

    # 6. Raw audio (no command prefix)
    assert parse_msg(b"just some random audio bytes")["command_type"] == "raw_audio"
